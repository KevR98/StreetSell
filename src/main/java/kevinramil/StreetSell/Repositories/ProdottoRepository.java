...
public interface ProdottoRepository extends JpaRepository<Prodotto, UUID> {
    ...
    @Query("SELECT COUNT(p) FROM Prodotto p WHERE p.venditore = :venditore AND p.statoProdotto = 'DISPONIBILE'")
    long countByVenditoreAndStatoProdotto(@Param("venditore") Utente venditore);
